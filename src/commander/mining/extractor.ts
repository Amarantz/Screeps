 ...
 */
export class ExtractorCommander extends Commander {
	...
	constructor(directive: DirectiveExtract, priority: number) {
		...
		this.priority += this.outpostIndex * CommanderPriority.remoteSKRoom.roomIncrement;
		this.drones = this.unit(Roles.drone);
		// Populate structures
	...
	private registerOutputRequests(): void {
		if (this.container) {
			if (_.sum(this.container.store) > 0.5 * this.container.storeCapacity ||
				...
				this.base.logisticsNetwork.requestOutput(this.container, {resourceType: 'all'});
			}
		}
	}
	...
	private handleDrone(drone: Unit): void {
