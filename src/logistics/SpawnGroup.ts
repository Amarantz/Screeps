import { onPublicServer, getAllBaseRooms, getCacheExpiration, minBy } from "utils/utils";
import HandOfNod, { SpawnRequest } from "mcv/handOfNod";
...
export class SpawnGroup {
	...
	constructor(initializer: SpawnGroupInitializer, settings: Partial<SpawnGroupSettings> = {}) {
		...
		Cobal.spawnGroups[this.ref] = this;
	}
	...
	private recalculateColonies() { // don't use settings when recalculating colonies as spawnGroups share memory
		const colonyRoomsInRange = _.filter(getAllBaseRooms(), room =>
	...
	init(): void {
		...
		const colonies = _.compact(_.map(this.colonyNames, name => Cobal.bases[name])) as Base[];
		const hatcheries = _.compact(_.map(colonies, base => base.handOfNod)) as HandOfNod[];
		const distanceTo = (hatchery: HandOfNod) => this.memory.distances[hatchery.pos.roomName] + 25;
		...
		for (const request of this.requests) {
			...
			const bestHatchery = minBy(okHatcheries, handOfNod => handOfNod.nextAvailability + distanceTo(handOfNod));
			if (bestHatchery) {
