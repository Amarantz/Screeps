...
export interface MoveOptions {
	...
	force?: boolean;							// whether to ignore Unit.blockMovement
...
export class Movement {
	 ...
	 */
	static goTo(creep: Unit, destination: HasPos | RoomPosition, options: MoveOptions = {}): number {
	 ...
	 */
	private static traversePortalWaypoint(creep: Unit, portalPos: RoomPosition): boolean {
	 ...
	 */
	private static crossPortal(creep: Unit, portalPos: RoomPosition): boolean {
	...
	private static getPushPriority(creep: Creep | Unit): number {
	...
	private static getPushDirection(pusher: Unit | Creep, pushee: Unit | Creep): DirectionConstant {
		...
		if (isUnit(pushee)) {
			...
			log.debug(`${pushee.name}@${pushee.pos.print} is not Unit! (Why?)`);
		}
		return pushee.pos.getDirectionTo(pusher);
	}

	private static findBlockingCreep(creep: Unit): Creep | undefined {
	...
	static pushCreep(creep: Unit, otherCreep: Creep | Unit): boolean {
		...
		if (isUnit(otherCreep)) {
			...
			log.debug(`${otherCreep.name}@${otherCreep.pos.print} is not Unit! (Why?)`);
			if (outcome == OK) {
	...
	static vacatePos(pos: RoomPosition, suicide = false): boolean {
		...
		const nearbyCreeps = _.compact(_.map(pos.findInRange(FIND_MY_CREEPS, 2),
											 creep => Cobal.units[creep.name])) as Unit[];
		...
		const blockingCreep = Cobal.units[creep.name];
	 ...
	 */
	static recursivePush(creep: Unit, excludePos: RoomPosition[] = []): RoomPosition | undefined {
	 ...
	 * Travel to a room
	 */
	static goToRoom(creep: Unit, roomName: string, options: MoveOptions = {}): number {
		options.range = 23;
	 ...
	 */
	static moveOffCurrentPos(creep: Unit): number | undefined {
	 ...
	 */
	static moveOnExit(creep: Unit): ScreepsReturnCode | undefined {
	 ...
	 */
	static moveOffExit(creep: Unit, avoidSwamp = true): ScreepsReturnCode {
		let swampDirection;
		const directions = [1, 3, 5, 7, 2, 4, 6, 8] as DirectionConstant[];
		for (const direction of directions) {
			const position = creep.pos.getPositionAtDirection(direction);
	 ...
	 */
	static moveOffExitToward(creep: Unit, pos: RoomPosition, detour = true): number | undefined {
	 ...
	 */
	static pairwiseMove(leader: Unit, follower: Unit, target: HasPos | RoomPosition,
	 ...
	 */
	static invasionMove(creep: Unit, destination: RoomPosition | HasPos, options: MoveOptions = {}): number {
		_.defaults(options, {
			ignoreRoads: true
		});
	 ...
	 */
	static kite(creep: Unit, avoidGoals: (RoomPosition | HasPos)[], options: MoveOptions = {}): number | undefined {
		_.defaults(options, {
			fleeRange   : 5,
	 ...
	 */
	static flee(creep: Unit, avoidGoals: (RoomPosition | HasPos)[],
	...
	private static serializeState(creep: Unit, destination: RoomPosition, state: MoveState, moveData: MoveData,
	...
	private static isStuck(creep: Unit, state: MoveState): boolean {
		let stuck = false;
		if (state.lastCoord !== undefined) {
