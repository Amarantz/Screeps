...
import {alignedNewline, bullet, leftArrow, rightArrow} from '../utils/stringConstants';
import {maxBy, minBy, onPublicServer, printRoomName} from '../utils/utils';
...
export class TraderJoe implements ITradeNetwork {
	...
	constructor() {
		this.memory = Mem.wrap(Memory.Cobal, 'trader', TraderMemoryDefaults, true);
	...
	private cleanUpInactiveOrders() {
		...
		const ordersToClean = _.filter(Game.market.orders, o =>
			...
			|| (o.roomName && !Cobal.bases[o.roomName]));							// order placed from dead colony
