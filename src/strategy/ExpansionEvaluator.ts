...
} from '../utils/Cartographer';
import {derefCoords} from '../utils/utils';
...
export class ExpansionEvaluator {
	...
	static refreshExpansionData(base: Base): void {
		// This only gets run once per base
		if (_.keys(base.memory.expansionData.possibleExpansions).length == 0
			|| Game.time > base.memory.expansionData.expiration) {
			// Generate a list of rooms which can possibly be settled in
			const nearbyRooms = Cartographer.recursiveRoomSearch(base.room.name, 5);
			...
			for (const roomName of possibleExpansions) {
				if (Cartographer.roomType(roomName) == ROOMTYPE_CONTROLLER) {
					base.memory.expansionData.possibleExpansions[roomName] = true;
				}
			}
		}
		// This gets run whenever function is called
		for (const roomName in base.memory.expansionData.possibleExpansions) {
			if (base.memory.expansionData.possibleExpansions[roomName] == true) {
				if (Memory.rooms[roomName]) {
					const expansionData = Memory.rooms[roomName][_RM.EXPANSION_DATA];
					if (expansionData == false) {
						base.memory.expansionData.possibleExpansions[roomName] = false;
					} else if (expansionData && expansionData.score) {
						base.memory.expansionData.possibleExpansions[roomName] = expansionData.score;
					}
				}
			}
		}
	...
	static computeExpansionData(room: Room, verbose = false): boolean {
		...
		for (const roomName in outpostSourcePositions) {
			...
			for (const position of sourcePositions) {
				...
				if (ret.incomplete || ret.path.length > Base.settings.maxSourceDistance) {
		...
		for (const roomName of roomsByScore) {
			if (sourceCount > Base.settings.remoteSourcesByLevel[8]) break;
