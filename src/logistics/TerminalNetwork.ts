...
import {alignedNewline, bullet, rightArrow} from '../utils/stringConstants';
import {maxBy, mergeSum, minBy, minMax} from '../utils/utils';
...
function baseOf(terminal: StructureTerminal): Base {
	return Cobal.bases[terminal.room.name];
}

function wantedAmount(base: Base, resource: ResourceConstant): number {
	return Abathur.stockAmount(resource) - (base.assets[resource] || 0);
}


/**
 * The terminal network controls inter-base resource transfers and requests, equalizing resources between rooms and
...
export class TerminalNetwork implements ITerminalNetwork {
	...
	constructor(terminals: StructureTerminal[]) {
		...
		this.memory = Mem.wrap(Memory.Cobal, 'terminalNetwork', TerminalNetworkMemoryDefaults);
	...
	/* Summarizes the total of all resources currently in a base store structure */
	private getAllAssets(): { [resourceType: string]: number } {
		return mergeSum(_.map(this.terminals, terminal => baseOf(terminal).assets));
	}
	...
	requestResource(receiver: StructureTerminal, resourceType: ResourceConstant, amount: number,
		...
		if (sender) {
			...
			Cobal.tradeNetwork.buy(receiver, resourceType, amount);
		}
	}
		...
		for (const resource in terminal.store) {
			...
			if (resource == RESOURCE_ENERGY) {
				...
				if (terminal.store[RESOURCE_ENERGY] > energyThreshold) {
					...
					const storage = baseOf(terminal).storage;
					...
					if (!storage || storage.energy >= storageEnergyCap) {
						...
						if (terminalNearCapacity) { // just get rid of stuff at high capacities
							const response = Cobal.tradeNetwork.sellDirectly(terminal, RESOURCE_ENERGY, amount, true);
							if (response == OK) return;
						} else {
							const response = Cobal.tradeNetwork.sell(terminal, RESOURCE_ENERGY, amount,
				...
				if (terminal.store[<ResourceConstant>resource]! > threshold) {
					const receiver = maxBy(this.terminals,
										   terminal => wantedAmount(baseOf(terminal),
																	<ResourceConstant>resource));
					if (receiver && wantedAmount(baseOf(receiver), <ResourceConstant>resource) > TERMINAL_MIN_SEND) {
						...
						if (terminalNearCapacity || terminal.store[<ResourceConstant>resource]! > 2 * threshold) {
							const response = Cobal.tradeNetwork.sellDirectly(terminal, <ResourceConstant>resource, 1000);
							if (response == OK) return;
						} else {
							const response = Cobal.tradeNetwork.sell(terminal, <ResourceConstant>resource, 10000);
							if (response == OK) return;
						}
					}
				}
	...
	private equalize(resourceType: ResourceConstant, terminals = this.terminals, verbose = false): void {
		...
		for (const [sender, receiver] of _.zip(senders, receivers)) {
			...
			if (senderAmount - receiverAmount < tolerance
				...
				continue; // skip if bases are close to equilibrium
			}
	...
	registerTerminalState(terminal: StructureTerminal, state: TerminalState): void {
		...
		baseOf(terminal).terminalState = state;
	...
	private handleTerminalState(terminal: StructureTerminal, state: TerminalState): void {
		for (const resourceType of RESOURCE_IMPORTANCE) {
			...
			// Terminal input state - request resources be sent to this base
			if (state.type == 'in' || state.type == 'in/out') {
				if (amount < targetAmount - tolerance) {
					// Request needed resources from most plentiful base
			...
			// Terminal output state - push resources away from this base
	...
	run(): void {
		...
		// }
			...
			if (Game.market.credits > TraderJoe.settings.market.energyCredits) {
				const averageEnergy = _.sum(this.terminals, terminal => baseOf(terminal).assets[RESOURCE_ENERGY] || 0) / this.terminals.length;
				if (averageEnergy < TerminalNetwork.settings.buyEnergyThreshold) {
					const poorestTerminal = minBy(this.terminals,
												  terminal => baseOf(terminal).assets[RESOURCE_ENERGY] || 0);
					if (poorestTerminal) {
						const amount = Energetics.settings.terminal.energy.tradeAmount;
						Cobal.tradeNetwork.maintainBuyOrder(poorestTerminal, RESOURCE_ENERGY, amount,
