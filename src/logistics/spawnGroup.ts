...
import {HandOfNod, SpawnRequest} from '../componets/HandOfNod';
import Mem from '../memory/Memory';
import {Pathing} from '../movement/Pathing';
import {getAllBasesRooms, getCacheExpiration, minBy, onPublicsServer} from '../utils/utils';

interface SpawnGroupMemory {
	bases: string[];
	distances: { [baseName: string]: number };
	routes: { [basesName: string]: { [roomName: string]: boolean } };
...
const DEFAULT_RECACHE_TIME = onPublicsServer() ? 2000 : 1000;
...
export class SpawnGroup {
	...
	constructor(initializer: SpawnGroupInitializer, settings: Partial<SpawnGroupSettings> = {}) {
		...
		// Compute stats
		this.colonyNames = _.filter(this.memory.bases,
		...
		global.Cobal.spawnGroups[this.ref] = this;
	}
	...
	private recalculateColonies() { // don't use settings when recalculating colonies as spawnGroups share memory
		const colonyRoomsInRange = _.filter(getAllBasesRooms(), room =>
	...
	init(): void {
		...
		const bases = _.compact(_.map(this.colonyNames, name => global.Cobal.bases[name])) as Base[];
		const handOfNods = _.compact(_.map(bases, base => base.handOfNod )) as HandOfNod[];
		const distanceTo = (handOfNod: HandOfNod) => this.memory.distances[handOfNod.pos.roomName] + 25;
		...
		for (const request of this.requests) {
			...
			const okHandOfNods = _.filter(handOfNods,
										  handOfNod => handOfNod.room.energyCapacityAvailable >= maxCost);
			// || this.settings.flexibleEnergy);
			const bestHandOfNod = minBy(okHandOfNods, HandOfNod => HandOfNod.nextAvailability + distanceTo(HandOfNod));
			if (bestHandOfNod) {
				bestHandOfNod.enqueue(request);
			} else {
