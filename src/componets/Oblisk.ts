    ...
    private registerEnergyRequests(): void {
        for( const tower of this.towers) {
            if(tower.energy < Oblisk.settings.requestThreshold){
                ...
                this.base.logisticsNetwork.requestInput(tower, {multiplier, dAmountdt})
            }
    ...
    private preventRampartDecay() {
        if(this.base.level < 7 && this.towers.length > 0) {
            const dyingRamparts = _.filter(this.room.ramparts, rampart => {
                rampart.hits < WorkerOverload.settings.barrierHits.critcal && this.base.roomPlanner.barrierPlanner.barrierShouldBeHere(rampart.pos);
