...
import { isCombatUnit } from '../declarations/typeGuards'
...
export class GoalFinder {
	...
	static skirmishGoals(creep: CombatUnit): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
	...
	static retreatGoals(creep: CombatUnit): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];
		const avoid: PathFinderGoal[] = [];

		const isHealer = CombatIntel.isHealer(creep);
		for (const friendly of creep.room.creeps) {
			if (CombatIntel.getHealPotential(friendly) > 0 || (isHealer && isCombatUnit(creep))) {
	...
	static healingGoals(healer: CombatUnit): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];
		const avoid: PathFinderGoal[] = [];
	...
	static structureGoals(creep: CombatUnit): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];
