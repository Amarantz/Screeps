...
/**
 * Commando is responsible for the evolution of the swarm and directs global production of minerals. Commando likes
...
export default class Commando {
	...
	constructor(base: Base) {
		this.base = base;
		this.memory = Mem.wrap(this.base.memory, 'abathur', AbathurMemoryDefaults);
	...
	private canBuyBasicMineralsForReaction(mineralQuantities: { [resourceType: string]: number }): boolean {
		...
		for (const mineral in mineralQuantities) {
			...
			if (Cobal.tradeNetwork.priceOf(<ResourceConstant>mineral) > maxPrice) {
				return false;
			}
		}
		return true;
	...
	private hasExcess(mineralType: ResourceConstant, excessAmount = 0): boolean {
		return this.assets[mineralType] - excessAmount > Commando.stockAmount(mineralType);
	}

	private someColonyHasExcess(mineralType: ResourceConstant, excessAmount = 0): boolean {
		return _.any(getAllBases(), base => base.commando.hasExcess(mineralType, excessAmount));
	}
	...
	getReactionQueue(verbose = false): Reaction[] {
		...
		this.memory.sleepUntil = Game.time + Commando.settings.sleepTime;
		return [];
	}
	...
	private buildReactionQueue(mineral: ResourceConstant, amount: number, verbose = false): Reaction[] {
		amount = minMax(amount, Commando.settings.minBatchSize, Commando.settings.maxBatchSize);
		if (verbose) console.log(`Commando@${this.base.room.print}: building reaction queue for ${amount} ${mineral}`);
		...
		for (const ingredient of this.ingredientsList(mineral)) {
			...
			productionAmount = Math.min(productionAmount, Commando.settings.maxBatchSize);
			...
			for (const index of [precursor1, precursor2]) {
				if (index != -1) {
					if (reactionQueue[index].amount == 0) {
						...
						reactionQueue[index].amount = minMax(reaction.amount, Commando.settings.minBatchSize,
