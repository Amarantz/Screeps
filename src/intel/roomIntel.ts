...
export class RoomIntel {
	...
	private static recordPermanentObjects(room: Room): void {
		...
		room.memory[_RM.CONSTROLLER] = room.controller ? {
			...
			[_RM_CTRL.RESERVATION]       : room.controller.reservation ?
										   {
											   ...
											   [_RM_CTRL.RES_TICKTOEND]: room.controller.reservation.ticksToEnd,
										   } : undefined,
			[_RM_CTRL.SAFEMODE]          : room.controller.safeMode,
			[_RM_CTRL.SAFEMODE_AVAILABLE]: room.controller.safeModeAvailable,
			[_RM_CTRL.SAFEMOD_COOLDOWN] : room.controller.safeModeCooldown,
		...
		if (room.controller && room.controller.owner) {
			room.memory[_RM.IMPORTANT_STRUCTUES] = {
			...
			room.memory[_RM.IMPORTANT_STRUCTUES] = undefined;
		}
		room.memory[_MEM.TICK] = Game.time;
	}
	...
	private static recordControllerInfo(controller: StructureController): void {
		const savedController = controller.room.memory[_RM.CONSTROLLER];
		if (savedController) {
			savedController[_RM_CTRL.RESERVATION] = controller.reservation ? {
				[_RM_CTRL.RES_USERNAME]  : controller.reservation.username,
				[_RM_CTRL.RES_TICKTOEND]: controller.reservation.ticksToEnd,
			} : undefined;
			savedController[_RM_CTRL.SAFEMODE] = controller.safeMode;
			savedController[_RM_CTRL.SAFEMOD_COOLDOWN] = controller.safeModeCooldown;
		}
	}

	static inSafeMode(roomName: string): boolean {
		if (!!Memory.rooms[roomName] && !!Memory.rooms[roomName][_RM.CONSTROLLER]) {
			const safemode = Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.SAFEMODE];
	...
	static safeModeCooldown(roomName: string): number | undefined {
		if (Memory.rooms[roomName] && Memory.rooms[roomName][_RM.CONSTROLLER] &&
			Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.SAFEMOD_COOLDOWN]) {
			const smcooldown = Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.SAFEMOD_COOLDOWN];
			const tick = Memory.rooms[roomName][_MEM.EXPIRATION];
			if (smcooldown && tick) {
	...
	private static recomputeScoreIfNecessary(room: Room): boolean {
		if (room.memory[_RM.EXPANTION_DATA] == false) { // room is uninhabitable or owned
			...
			if (!room.memory[_RM.EXPANTION_DATA] || Math.random() < SCORE_RECALC_PROB) {
	...
	static roomOwnedBy(roomName: string): string | undefined {
		if (Memory.rooms[roomName] && Memory.rooms[roomName][_RM.CONSTROLLER] &&
			Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.OWNER]) {
			if (Game.time - (Memory.rooms[roomName][_MEM.TICK] || 0) < 25000) { // ownership expires after 25k ticks
				return Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.OWNER];
			}
		}
	}
	...
	static roomReservedBy(roomName: string): string | undefined {
		if (Memory.rooms[roomName] && Memory.rooms[roomName][_RM.CONSTROLLER] &&
			Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.RESERVATION]) {
			if (Game.time - (Memory.rooms[roomName][_MEM.TICK] || 0) < 10000) { // reservation expires after 10k ticks
				return Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.RESERVATION]![_RM_CTRL.RES_USERNAME];
			}
		}
	}
	...
	static roomReservationRemaining(roomName: string): number {
		if (Memory.rooms[roomName] && Memory.rooms[roomName][_RM.CONSTROLLER] &&
			Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.RESERVATION]) {
			const ticksToEnd = Memory.rooms[roomName][_RM.CONSTROLLER]![_RM_CTRL.RESERVATION]![_RM_CTRL.RES_TICKTOEND];
	...
	static run(): void {
		...
		for (const name in Game.rooms) {
			...
			if (Cobal.baseMap[room.name]) { // if it is an owned or outpost room
