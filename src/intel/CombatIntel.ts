...
export class CombatIntel {
	...
	private findBestExit(matrix: CostMatrix, towers: StructureTower[],
		...
		// Figure out possible exits to go from enemy room back to base in a reasonable amount of time
	...
	findSwarmAssemblyPointInColony(clearance: { width: number, height: number }, swarmIndex = 0): RoomPosition {
		// let ret = Pathing.findSwarmPath(this.base.pos, this.directive.pos, clearance.width, clearance.height,
		// 								{ignoreCreeps: true});
	 ...
	 */
	static isHealer(zerg: Creep | Unit): boolean {
