...
export class DirectiveTerminalRebuildState extends Directive {
	...
	refresh() {
		...
		if (this.terminal) {
			Cobal.terminalNetwork.registerTerminalState(this.terminal, TERMINAL_STATE_REBUILD);
		}
		if (Game.time % 25 == 0) {
	...
	run() {
		...
		if (!this.base || !this.terminal || Game.time > (this.memory[_MEM.TICK] || 0) + REBUILD_STATE_TIMEOUT) {
			this.remove();
		}
	}
}
