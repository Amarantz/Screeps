...
export class DefenseNPCCommander extends Commander {

	guards: CombatUnit[];

	static requiredRCL = 3;

	constructor(directive: DirectiveGuard, priority = CommanderPriority.outpostDefense.guard) {
		super(directive, 'guard', priority);
		this.guards = this.combatUnit(Roles.guardMelee);
	}
	...
	private findAttackTarget(guard: CombatUnit): Creep | Structure | undefined | null {
	...
	private combatActions(guard: CombatUnit, target: Creep | Structure): void {
	...
	private handleGuard(guard: CombatUnit): void {
