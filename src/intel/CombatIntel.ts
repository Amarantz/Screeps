...
export class CombatIntel {
	...
	private findBestExit(matrix: CostMatrix, towers: StructureTower[],
		...
		const ret = Pathing.findPath(this.base.pos, destination.pos, {range: 1});
		if (!ret.incomplete) {
		...
		if (!bestExit) {
			...
			for (const direction in exitData) {
				...
				const allowedRooms = Pathing.findRoute(this.base.name, roomName);
	...
	findSimpleSiegeFallback(): RoomPosition {
		const ret = Pathing.findPath(this.base.pos, this.directive.pos, {range: 23});
		if (ret.incomplete) {
	...
	findSwarmAssemblyPointInColony(clearance: { width: number, height: number }, swarmIndex = 0): RoomPosition {
		...
		// 								{ignoreCreeps: true});
		const ret = Pathing.findPath(this.base.pos, this.directive.pos, {ignoreCreeps: true});
		const path = ret.path.reverse();
		const acceptablePositions = _.filter(path, pos => pos.roomName == this.base.name && pos.rangeToEdge > 1);
		const swarmSize = Math.max(clearance.width, clearance.height);
		const posIndex = (swarmSize + 1) * swarmIndex;
	...
	static getHealAmount(creep: Creep | Unit): number {
		return HEAL_POWER * this.getHealPotential(toCreep(creep));
	}

	static getRangedHealAmount(creep: Creep | Unit): number {
	 ...
	 */
	static isHealer(unit: Creep | Unit): boolean {
		const creep = toCreep(unit);
		const healParts = _.filter(unit.body, part => part.type == HEAL).length;
		const attackParts = _.filter(unit.body, part => part.type == ATTACK).length;
		const rangedAttackParts = _.filter(unit.body, part => part.type == RANGED_ATTACK).length;
	...
	static getAttackDamage(creep: Creep | Unit): number {
	...
	static getRangedAttackDamage(creep: Creep | Unit): number {
	...
	static getDismantleDamage(creep: Creep | Unit): number {
	...
	static getMassAttackDamageTo(attacker: Creep | Unit, target: Creep | Structure): number {
		...
		return rangedMassAttackPower * this.getRangedAttackPotential(isUnit(attacker) ? attacker.creep : attacker);
	}

	/**
	 * Total damage to enemy creeps done by attacker.rangedMassAttack()
	 */
	static getMassAttackDamage(attacker: Creep | Unit, targets = attacker.room.hostiles, checkRampart = true): number {
	 ...
	 */
	static rating(creep: Creep | Unit): number {
	 ...
	 * Heal potential of self and possible healer neighbors
	 */
	static maxFriendlyHealingTo(friendly: Creep | Unit): number {
	 ...
	 */
	static predictedDamageAmount(attacker: Creep | Unit, target: Creep, attackType: 'attack' | 'rangedAttack',
