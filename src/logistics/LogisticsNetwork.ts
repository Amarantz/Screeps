...
export class LogisticsNetwork {
	...
	constructor(base: Base) {
		this.memory = Mem.wrap(base.memory, 'logisticsNetwork', LogisticsNetworkMemoryDefaults);
		this.requests = [];
		this.targetToRequest = {};
		this.base = base;
		// this.transporters = _.filter(base.getCreepsByRole(TransporterSetup.role),
		// 							 creep => !creep.spawning &&
		// 									  creep.carryCapacity >= LogisticsNetwork.settings.carryThreshold);
		this.buffers = _.compact([base.storage!, base.terminal!]);
		this.cache = {
			nextAvailability         : {},
		...
		// for (let room of this.base.rooms) {
		// 	this.logisticPositions[room.name] = _.map([...room.storageUnits, ...room.links], s => s.pos);
		// }
	}

	refresh(): void {
		this.memory = Mem.wrap(this.base.memory, 'logisticsNetwork', LogisticsNetworkMemoryDefaults);
		this.requests = [];
		this.targetToRequest = {};
	...
	requestInput(target: LogisticsTarget, opts = {} as RequestOptions): void {
		...
		if (target.room != this.base.room) {
			log.warning(`${target.ref} at ${target.pos.print} is outside base room; shouldn't request!`);
			return;
		}
	...
	static targetingTransporters(target: LogisticsTarget, excludedTransporter?: Unit): Unit[] {
		const targetingZerg = _.map(target.targetBy, name => Cobal.units[name]);
	...
	bufferChoices(transporter: Unit, request: LogisticsRequest): {
		...
		if (amount > 0) { // requestInput instance, needs refilling
			...
			// 										 this.base.linkNetwork.getDropoffAvailability(link));
	 ...
	 */
	summarize(): void {
		// console.log(`Summary of logistics group for ${this.base.name} at time ${Game.time}`);
		let info = [];
