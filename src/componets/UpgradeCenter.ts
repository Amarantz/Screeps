...
const UpgradeCenterMemoryDefaults: UpgradeCenterMemory = {
...
export class UpgradeCenter extends Component {
	...
	constructor(base: Base, terminal: StructureTerminal) {
		super(base, terminal, 'evolutionChamber');
		this.memory = Mem.wrap(this.base.memory, 'evolutionChamber', UpgradeCenterMemoryDefaults);
		...
		const restrictedLabs = this.base.bunker ?
							   _.filter(this.labs, lab => lab.pos.findInRange(this.base.spawns, 1).length > 0) :
		...
		if (this.base.bunker && this.base.labs.length == 10) {
		...
		// this.boostQueue = {};
		this.neededBoosts = {};
		if (this.base.commandCenter && this.base.layout == 'twoPart') {
			// in two-part layout, evolution chamber shares a common request group with command center
			this.transportRequests = this.base.commandCenter.transportRequests;
		} else {
			// otherwise (in bunker layout), it uses colony/hatchery transport requests
			this.transportRequests = this.base.transportRequests;
		}
	}

	refresh() {
		this.memory = Mem.wrap(this.base.memory, 'UpgradeCenter', UpgradeCenterMemoryDefaults);
		$.refreshRoom(this);
		...
		switch (this.memory.status) {
			case LabStatus.Idle:
				if (this.memory.activeReaction) {
					...
					log.info(`${this.base.room.print}: starting synthesis of ${ing1} + ${ing2} ${rightArrow} ` +
			...
			case LabStatus.AcquiringMinerals: // "We acquire more mineralzzz"
				const missingIngredients = this.base.abathur.getMissingBasicMinerals([this.memory.activeReaction!]);
	...
	canBoost(body: BodyPartDefinition[], boostType: _ResourceConstantSansEnergy): boolean {
		const boostAmount = UpgradeCenter.requiredBoostAmount(body, boostType);
		if (this.base.assets[boostType] >= boostAmount) {
			...
			return (Game.market.credits > TraderJoe.settings.market.boostCredits +
					boostAmount * global.Cobal.tradeNetwork.priceOf(boostType));
		}
	}

	/* Request boosts sufficient to fully boost a given creep to be added to the boosting queue */
	requestBoost(creep: Unit, boostType: _ResourceConstantSansEnergy): void {

		// Add the required amount to the neededBoosts
		const boostAmount = UpgradeCenter.requiredBoostAmount(creep.body, boostType);
	...
	init(): void {
		...
		if (this.memory.reactionQueue.length == 0) {
			this.memory.reactionQueue = this.base.abathur.getReactionQueue();
		}
	...
	run(): void {
		...
		for (const resourceType in this.neededBoosts) {
			const needAmount = Math.max(this.neededBoosts[resourceType] - (this.base.assets[resourceType] || 0), 0);
			if (needAmount > 0) {
		...
		const missingBasicMinerals = this.base.abathur.getMissingBasicMinerals(queue);
	...
	// private stats(): void {
	// 	Stats.log(`colonies.${this.base.name}.evolutionChamber.totalProduction`, this.memory.stats.totalProduction);
	...
	// 	Stats.log(`colonies.${this.base.name}.evolutionChamber.avgUsage`, this.memory.stats.avgUsage);
	// }
