...
export class CombatTargeting {
	...
	static findBestStructureTargetInRange(unit: Unit, range: number, allowUnowned = true): Structure | undefined {
		...
		if (allowUnowned && nearbyStructures.length == 0 && !global.Cobal.colonyMap[unit.room.name]) {
	...
	static findClosestPrioritizedStructure(zerg: Unit, checkReachable = false): Structure | undefined {
