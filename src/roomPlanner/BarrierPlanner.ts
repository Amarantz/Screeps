...
export class BarrierPlanner {
	...
	private computeBarrierPositions(hatcheryPos: RoomPosition, commandCenterPos: RoomPosition,
		...
		// Get Min cut
		const barrierCoords = getCutTiles(this.base.name, rectArray, true, 2, false);
		return _.map(barrierCoords, coord => new RoomPosition(coord.x, coord.y, this.base.name));
	}
	...
	run(): void {
		if (this.roomPlanner.active) {
			...
			if (!this.roomPlanner.memory.relocating && this.base.level >= BarrierPlanner.settings.buildBarriersAtRCL
				...
				if (this.base.layout == 'bunker' && this.base.level >= 7) {
