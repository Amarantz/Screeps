...
import { Reaction } from '../resources/commando';
...
export default class EvolutionChamber extends MCV {

	terminal: StructureTerminal;							// The base terminal
	terminalNetwork: TerminalNetwork;						// Reference to Cobal.terminalNetwork
	labs: StructureLab[];									// Base labs
	...
	constructor(base: Base, terminal: StructureTerminal) {
		...
		this.memory = Mem.wrap(this.base.memory, 'evolutionChamber', EvolutionChamberMemoryDefaults);
		...
		const restrictedLabs =_.filter(this.labs, lab => lab.pos.findInRange(this.base.spawns, 1).length > 0);
		...
		// this.boostQueue = {};
		this.neededBoosts = {};
		// otherwise (in bunker layout), it uses base/hatchery transport requests
	...
	private initLabStatus(): void {
		...
		switch (this.memory.status) {
			...
			case LabStatus.AcquiringMinerals: // "We acquire more mineralzzz"
				const missingIngredients = this.base.commando.getMissingBasicMinerals([this.memory.activeReaction!]);
	...
	canBoost(body: BodyPartDefinition[], boostType: _ResourceConstantSansEnergy): boolean {
		...
		if (this.base.assets[boostType] >= boostAmount) {
			// Does this base have the needed resources already?
			return true;
			...
			return (Game.market.credits > TraderJoe.settings.market.boostCredits +
					boostAmount * Cobal.tradeNetwork.priceOf(boostType));
		}
	}
	...
	init(): void {
		...
		if (this.memory.reactionQueue.length == 0) {
			this.memory.reactionQueue = this.base.commando.getReactionQueue();
		}
	...
	run(): void {
		...
		const missingBasicMinerals = this.base.commando.getMissingBasicMinerals(queue);
	...
	// 	let {x, y} = coord;
	// 	const height = 2;
	// 	const titleCoords = Visualizer.section(`${this.base.name} Evolution Chamber`,
