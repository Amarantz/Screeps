...
interface CombatUnitMemory extends CreepMemory {
    recovering: boolean;
    lastInDanger: number;
    partner?: string;
    swarm?: string;
...
export const DEFAULT_SQUAD_TICK_DIFFERENCE = 500;

export class CombatUnit extends Unit {

	memory: CombatUnitMemory;
	...
	findPartner(partners: CombatUnit[], tickDifference = DEFAULT_PARTNER_TICK_DIFFERENCE): CombatUnit | undefined {
	...
	findSwarm(partners: CombatUnit[], maxByRole: { [role: string]: number },
			  tickDifference = DEFAULT_SQUAD_TICK_DIFFERENCE): string | undefined {
	...
	autoBunkerCombat(roomName: string, verbose = false) {
		...
		const siegingCreeps = this.room.hostiles.filter(creep =>
			_.any(creep.pos.neighbors, pos => this.base && insideBunkerBounds(pos, this.base)));
