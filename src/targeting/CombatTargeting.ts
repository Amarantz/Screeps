...
export class CombatTargeting {
	...
	/**
	 * Finds the best target within a given range that a unit can currently attack
	 */
	static findBestCreepTargetInRange(unit: Unit, range: number, targets = unit.room.hostiles): Creep | undefined {
		const nearbyHostiles = _.filter(targets, c => unit.pos.inRangeToXY(c.pos.x, c.pos.y, range));
	 ...
	 * Finds the best target within a given range that a unit can currently attack
	 */
	static findBestStructureTargetInRange(unit: Unit, range: number, allowUnowned = true): Structure | undefined {
		let nearbyStructures = _.filter(unit.room.hostileStructures,
										s => unit.pos.inRangeToXY(s.pos.x, s.pos.y, range));
		// If no owned structures to attack and not in colony room or outpost, target unowned structures
		if (allowUnowned && nearbyStructures.length == 0 && !Cobal.baseMap[unit.room.name]) {
			nearbyStructures = _.filter(unit.room.structures,
										s => unit.pos.inRangeToXY(s.pos.x, s.pos.y, range));
		}
	 ...
	 */
	static findTarget(unit: Unit, targets = unit.room.hostiles): Creep | undefined {
		return maxBy(targets, function(hostile) {
			if (hostile.hitsPredicted == undefined) hostile.hitsPredicted = hostile.hits;
			if (hostile.pos.lookForStructure(STRUCTURE_RAMPART)) return false;
			return hostile.hitsMax - hostile.hitsPredicted + CombatIntel.getHealPotential(hostile)
				   - 10 * unit.pos.getMultiRoomRangeTo(hostile.pos); // compute score
		});
	}
	...
	static findClosestHostile(unit: Unit, checkReachable = false, ignoreCreepsAtEdge = true): Creep | undefined {
		if (unit.room.hostiles.length > 0) {
			let targets: Creep[];
			if (ignoreCreepsAtEdge) {
				targets = _.filter(unit.room.hostiles, hostile => hostile.pos.rangeToEdge > 0);
			} else {
				targets = unit.room.hostiles;
			}
			if (checkReachable) {
				const targetsByRange = _.sortBy(targets, target => unit.pos.getRangeTo(target));
				return _.find(targetsByRange, target => Pathing.isReachable(unit.pos, target.pos, unit.room.barriers));
			} else {
				return unit.pos.findClosestByRange(targets) as Creep | undefined;
			}
		}
	}
	...
	static findClosestHurtFriendly(healer: Unit): Creep | null {
		return healer.pos.findClosestByRange(_.filter(healer.room.creeps, creep => creep.hits < creep.hitsMax));
	}

	/**
	 * Finds the best (friendly) target in range that a unit can currently heal
	 */
	static findBestHealingTargetInRange(healer: Unit, range = 3, friendlies = healer.room.creeps): Creep | undefined {
	...
	static findClosestPrioritizedStructure(unit: Unit, checkReachable = false): Structure | undefined {
		for (const structureType of AttackStructurePriorities) {
			const structures = _.filter(unit.room.hostileStructures, s => s.structureType == structureType);
			if (structures.length == 0) continue;
			if (checkReachable) {
				const closestReachable = this.findClosestReachable(unit.pos, structures) as Structure | undefined;
				if (closestReachable) return closestReachable;
			} else {
				return unit.pos.findClosestByRange(structures) as Structure | undefined;
			}
		}
	}
